...
export class OrderService {
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    const copy: IOrder = Object.assign({}, order, {
      createTime: order.createTime != null && order.createTime.isValid() ? order.createTime.toJSON() : null,
      updateTime: order.updateTime != null && order.updateTime.isValid() ? order.updateTime.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        order.createTime = order.createTime != null ? moment(order.createTime) : null;
        order.updateTime = order.updateTime != null ? moment(order.updateTime) : null;
      });
    }
    return res;
  }
